...
export async function renderCurrentWeather(data) {
  try {
    ...
    const today = new Date((data.dt + data.timezone) * 1000);

    // Получаем день недели на русском языке:
    const firstDayOfWeek = today.toLocaleDateString("ru-RU", {
      weekday: "long",
      timeZone: "UTC",
    });
    ...
    const currentContent = `
                            ...
                            <div class="current__weather">
                              <p class="current__temperature">${
                                (Math.round(data.main.temp) > 0 ? "+" : "") + Math.round(data.main.temp) + "&deg"
                              };</p>
                              <div class="current-perceived">
                                ...
                                <p class="current-perceived__temperature">Ощущается как  ${
                                  (Math.round(data.main.feels_like) > 0 ? "+" : "") + Math.round(data.main.feels_like) + "&deg"
                                };</p>
                              </div>
                              <div class="weather-conditions">
                                <div class="weather-condition">
                                  <div class="weather-condition__icon-wrapper">
                                    <svg class ="weather-condition__icon" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-wind-icon lucide-wind" aria-hidden="true"><path d="M12.8 19.6A2 2 0 1 0 14 16H2"/><path d="M17.5 8a2.5 2.5 0 1 1 2 4H2"/><path d="M9.8 4.4A2 2 0 1 1 11 8H2"/></svg>
                                  </div>
                                ...
                                <div class="weather-condition">
                                  <div class="weather-condition__icon-wrapper">
                                    <svg class ="weather-condition__icon" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-droplet-icon lucide-droplet" aria-hidden="true"><path d="M12 22a7 7 0 0 0 7-7c0-2-1-3.9-3-5.5s-3.5-4-4-6.5c-.5 2.5-2 4.9-4 6.5C6 11.1 5 13 5 15a7 7 0 0 0 7 7z"/></svg>
                                  </div>
                                  <p class="weather-condition__value">${
    ...
    current.insertAdjacentHTML("beforeend", currentContent);

    return data.name;
  } catch (error) {
    console.error("Ошибка запроса текущей погоды", error.message);
  }
}
